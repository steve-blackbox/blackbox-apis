...
router.get('/', (req, res) => {
    ...
    res.status(200).send(`
        ...
        <html lang="en">
        <head>
            <meta charset="UTF-8"><title>🟢 CORE ENGINE 01 — FORM-SHIELD LIVE MONITOR</title>
        ...
        <body>
            <div class="box">
                <h2>⚡ ROBOT 01 : FORM-SHIELD</h2>
                <p>Statut du réacteur : <span class="green">● ONLINE & APPARENT</span></p>
                <p>Raccordement : <span class="blue">://blackbox-apis.com</span></p>
                <p style="color: #9ca3af;">[SYSTEM LOG] Protection anti-spam invisible JavaScript armée en amont de la base de données.</p>
            </div>
        </body>
        </html>
    `);
...
router.post('/process', (req, res) => {
    ...
    res.status(200).json({
        success: true,
        protected: true,
        robot: "Robot 01 - Form Shield",
        engine: "Centaure v6.7",
        metrics: { status: "SPAM_BLOCKED" },
