...
// 🩹 Migration douce : ajoute les colonnes d'abonnement si la base existait déjà
...
if (!existingLicenseColumns.includes('stripe_subscription_id')) {
    db.exec('ALTER TABLE licenses ADD COLUMN stripe_subscription_id TEXT');
}
if (!existingLicenseColumns.includes('stripe_customer_id')) {
    db.exec('ALTER TABLE licenses ADD COLUMN stripe_customer_id TEXT');
}
...
function saveLicense({ licenseKey, email, plan, stripeSubscriptionId, stripeCustomerId }) {
    db.prepare(`
        INSERT INTO licenses (license_key, email, plan, active, stripe_subscription_id, stripe_customer_id)
        VALUES (?, ?, ?, 1, ?, ?)
        ON CONFLICT(license_key) DO UPDATE SET
            active = 1,
            stripe_subscription_id = excluded.stripe_subscription_id,
            stripe_customer_id = excluded.stripe_customer_id
    `).run(licenseKey, email || null, plan || null, stripeSubscriptionId || null, stripeCustomerId || null);
}
...
// 🚫 Désactive la ou les licences liées à un abonnement Stripe annulé/impayé
// (appelé depuis le webhook sur customer.subscription.deleted / invoice.payment_failed)
function deactivateLicensesBySubscription(stripeSubscriptionId) {
    if (!stripeSubscriptionId) return 0;
...
module.exports = {
    ...
    deactivateLicensesBySubscription,
};
