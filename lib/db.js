...
// 🩹 Migration douce : ajoute les colonnes d'abonnement Paddle si la base existait déjà
// avant le passage au modèle mensuel récurrent (évite de perdre les licences existantes).
const existingLicenseColumns = db.prepare("PRAGMA table_info(licenses)").all().map((c) => c.name);
if (!existingLicenseColumns.includes('paddle_subscription_id')) {
    db.exec('ALTER TABLE licenses ADD COLUMN paddle_subscription_id TEXT');
}
if (!existingLicenseColumns.includes('paddle_customer_id')) {
    db.exec('ALTER TABLE licenses ADD COLUMN paddle_customer_id TEXT');
}
// 🩹 Migration douce : colonne "category" pour la restriction par catégorie du plan
// SOLO CORE (1 catégorie sur 3). Reste NULL pour les licences LABS ALL-ACCESS (accès total).
if (!existingLicenseColumns.includes('category')) {
    db.exec('ALTER TABLE licenses ADD COLUMN category TEXT');
}
...
// 💾 Enregistre une licence de façon permanente (appelé à chaque vente validée par Paddle).
function saveLicense({ licenseKey, email, plan, category, paddleSubscriptionId, paddleCustomerId }) {
    db.prepare(`
        INSERT INTO licenses (license_key, email, plan, category, active, paddle_subscription_id, paddle_customer_id)
        VALUES (?, ?, ?, ?, 1, ?, ?)
    ...
    `).run(licenseKey, email || null, plan || null, category || null, paddleSubscriptionId || null, paddleCustomerId || null);
}
...
// 🔎 Récupère le plan + la catégorie d'une licence (sert à restreindre l'accès SOLO CORE
// à une seule catégorie de robots — voir requireLicense dans server.js).
function getLicenseByKey(licenseKey) {
    return db.prepare('SELECT license_key, plan, category, active FROM licenses WHERE license_key = ?').get(licenseKey) || null;
}
...
// 🔄 Recharge plan + catégorie de toutes les licences actives (cache in-memory
// "licenseMeta" utilisé par requireLicense pour la restriction par catégorie).
function getAllActiveLicenseMeta() {
    return db.prepare('SELECT license_key, plan, category FROM licenses WHERE active = 1').all();
}

// 🚫 Désactive la ou les licences liées à un abonnement Paddle annulé ou en retard de
// paiement (subscription.canceled / subscription.past_due).
function deactivateLicensesByPaddleSubscription(paddleSubscriptionId) {
    if (!paddleSubscriptionId) return 0;
    const result = db.prepare(`
        UPDATE licenses SET active = 0 WHERE paddle_subscription_id = ?
    `).run(paddleSubscriptionId);
    return result.changes;
}

// 🎟️ CLÉ DÉMO PUBLIQUE (collection Postman publiée sur l'API Network) : ré-insérée
// automatiquement à chaque démarrage du serveur (voir seedDemoLicense ci-dessous), donc
// elle survit même si le disque persistant Render venait à être réinitialisé. Le volume
// est strictement limité côté server.js (requireLicense) — jamais un accès illimité.
const DEMO_LICENSE_KEY = process.env.DEMO_LICENSE_KEY || 'BB-DEMO-POSTMAN-PUBLIC01';

function seedDemoLicense() {
    saveLicense({
        licenseKey: DEMO_LICENSE_KEY,
        email: 'demo@blackbox-apis.com',
        plan: 'demo',
        category: null, // accès aux 12 robots, mais soumis au quota journalier strict
    });
}
...
module.exports = {
    ...
    isValidLicense,
    getLicenseByKey,
    getAllActiveLicenseKeys,
    getAllActiveLicenseMeta,
    deactivateLicensesByPaddleSubscription,
    DEMO_LICENSE_KEY,
    seedDemoLicense,
};
