/**
 * 🤖 BLACKBOX ROBOT AUTOMATE — CORE ENGINE 03
 * 🛡️ SERVICE : LINK-PURGE GATEWAY LAYER
...
router.get('/', (req, res) => {
    ...
    res.status(200).send(`
        ...
        <html lang="en">
        <head>
            <meta charset="UTF-8"><title>🟢 CORE ENGINE 03 — LINK-PURGE LIVE MONITOR</title>
        ...
        <body>
            <div class="box">
                <h2>⚡ ROBOT 03 : LINK-PURGE GATEWAY</h2>
                <p>Statut du réacteur : <span class="green">● ONLINE & APPARENT</span></p>
                <p>Raccordement : <span class="blue">://blackbox-apis.com</span></p>
                <p style="color: #9ca3af;">[SYSTEM LOG] Nettoyeur d'URls malicieuses armé. Tracking codes éliminés en amont.</p>
            </div>
        </body>
        </html>
    `);
...
router.post('/process', (req, res) => {
    ...
    res.status(200).json({
        success: true,
        purged: true,
        robot: "Robot 03 - Link Purge",
        engine: "Centaure v6.7",
        metrics: { status: "TRACKERS_DELETED" },
